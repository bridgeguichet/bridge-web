...
import { AlertTriangle, Ban, Briefcase, CheckCircle, FileText, Scale, Shield, Users } from "lucide-react";
  ...
  const renderSectionContent = (sectionId: string) => {
    ...
    return (
      <div className="space-y-4">
        ...
        {section.subtitle && <p className="font-semibold text-foreground mt-4">{section.subtitle}</p>}
        ...
        {section.penalty && <p className="text-muted-foreground ml-6">• {section.penalty}</p>}

        {section.duration && <p className="text-muted-foreground mt-4">{section.duration}</p>}

        {section.note && <p className="text-muted-foreground mt-4">{section.note}</p>}
    ...
    <div className="min-h-screen bg-background">
      <section className="relative bg-secondary py-20 text-white md:py-24">
        <div className="container mx-auto max-w-4xl px-6 text-center">
          <p className="mb-4 text-sm font-medium uppercase tracking-widest text-primary">{t("cguPage.heroLabel")}</p>
          <h1 className="mb-6 font-[batangas] text-4xl font-bold md:text-6xl">{t("cguPage.heroTitle")}</h1>
          <p className="mx-auto max-w-2xl text-base leading-relaxed text-white/70 md:text-lg">
            {t("cguPage.heroDesc")}
          </p>
          <p className="mt-4 text-sm text-white/60">{t("cguPage.lastUpdate")}</p>
        </div>
      </section>
      ...
      <section className="bg-white py-16 md:py-20">
        <div className="container mx-auto max-w-4xl px-6">
          <nav className="mb-12 rounded-2xl border border-border bg-muted/30 p-6">
            <h2 className="mb-4 text-lg font-semibold text-foreground">{t("cguPage.tableOfContents")}</h2>
          ...
          <div className="mt-16 rounded-2xl border border-border bg-muted/30 p-8">
            <h3 className="mb-4 text-xl font-semibold text-foreground">{t("cguPage.contact.title")}</h3>
            <p className="mb-4 text-muted-foreground">{t("cguPage.contact.description")}</p>
            <div className="space-y-2 text-sm">
              ...
              <p className="text-muted-foreground">
                <strong className="text-foreground">{t("cguPage.contact.address")}:</strong>{" "}
